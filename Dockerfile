...
ADD https://s3.amazonaws.com/influxdb/influxdb_0.9.4.2_amd64.deb /tmp/influxdb_0.9.4.2_amd64.deb
RUN dpkg -i /tmp/influxdb_0.9.4.2_amd64.deb
ADD https://s3.amazonaws.com/get.influxdb.org/chronograf/chronograf_0.2.0_amd64.deb /tmp/chronograf_0.2.0_amd64.deb
RUN dpkg -i /tmp/chronograf_0.2.0_amd64.deb
RUN mkdir -p /opt/influxdb/instance/etc
COPY conf/influx.conf /opt/influxdb/instance/etc/
COPY conf/chronograf-config.toml /opt/chronograf/
